...
Both technologies are backed by rigorous international testing. Vision Energy International assists consultants and contractors in selecting the optimal protection strategy based on structural geometry, site constraints, and local authority requirements.`,
  ...
  // 4. Seed Product SKUs for all 57 categories (PUBLISHED status)
  for (const cat of categoriesRaw) {
    const groupPrefix = cat.code.split("-")[0];
    const groupLabel = GROUP_LABELS[groupPrefix] || groupPrefix;
    const mainCategory =
      ["LP", "ER", "LT", "CM", "CB", "CT", "EL"].includes(groupPrefix)
        ? "Electrical"
        : ["ME", "HW"].includes(groupPrefix)
        ? "Mechanical"
        : ["EN"].includes(groupPrefix)
        ? "Solar"
        : groupLabel;
    const imageUrl = `/product-catalouge/${cat.code}.jpeg`;
    const imageAlt = `${cat.title} (${cat.code})`;
    ...
    await (prisma as any).product.upsert({
      ...
      update: {
        title: cat.title,
        description: cat.description,
        includes: cat.families,
        category: mainCategory,
        subcategoryGroup: groupLabel,
        sortOrder: cat.sortOrder,
        status: "PUBLISHED",
        imageUrl,
        imageAlt,
  ...
  console.log(`Seeded ${categoriesRaw.length} product SKUs into Product table (upsert).`);
